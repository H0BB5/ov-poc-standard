...
We have since run this entire suite inside a real Intel TDX trust domain and on
an identical non-confidential instance as a control, and the result corrects the
caveat we used to attach here. We said the omitted enclave cost meant the real
number would be \emph{bigger}. End to end it is \emph{smaller}:
\SI{160.05}{\micro\second} on the trust domain against
\SI{201.47}{\micro\second} below, because a Xeon Platinum 8481C signs Ed25519
in \SI{36}{\micro\second} where the M2~Max takes \SI{84}{\micro\second}.
The trust domain does cost extra---\textbf{5.1\%} against the control---but it
is swamped by the platform difference, and the honest summary is that these
numbers were conservative rather than optimistic. Every result below that is
\emph{not} a timing---the false-rejection rate, the detection rates, the
frontier and the Merkle figures---is bit-identical across laptop, control and
trust domain, which establishes as measured something this section previously
had to assume. Everything below: Apple M2 Max, Python 3.11, one core, 5{,}000
...
The enclave is pretend here. A real deployment pays 5.1\% for the trust domain,
measured, and \SI{39.5}{\milli\second} for each hardware quote, which is the
cost that actually matters and is in no number in this section. It does
\emph{not} pay more end to end: on server hardware the same pipeline runs at
\SI{160}{\micro\second}. Everything runs in one process, so no network or IPC
...
\paragraph{Measured twice, by different means.} The figure above is the
trust-domain overhead on one pipeline step. We have separately run the whole
benchmark suite of \S\ref{sec:eval} on a second pair of instances, TDX on
against TDX off, and it puts the end-to-end penalty at \textbf{5.1\%} where
this experiment puts it at 6.2\%. Two measurements of different quantities, by
different methods, on different machines, agreeing to about a percentage point.
That is the strongest evidence we have for the claim the design leans on
hardest---that running inside a trust domain is nearly free---and it is worth
more than either number alone. The same run also establishes that every result
in \S\ref{sec:eval} which is not a timing is bit-identical inside and outside
the trust domain.
